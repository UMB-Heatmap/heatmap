...
class Vis_Distribution(VisualizationInterface):
    def __init__(self, accessor, params):
        super().__init__(accessor, params)
        self['debug'] = False
        self['isScaleColorMap'] = True # vis.getBoolFromInput("Scale Color Map? (Y/N): ")
        os.system('clear')
        self['useGif'] = True

    def getParamList(self):
        return ['candidates', 'iterations', 'numberPerIteration', 'normalize', 'loopGif', 'gifDuration', 'colorMap']

    def getOptionInput(self, option):
        match option:
            case 'candidates':
                return InputHandler.getIntFromInput("Number of Candidates: ")
            case 'iterations':
                return InputHandler.getIntFromInput("Number of Iterations: ")
            case 'numberPerIteration':
                return InputHandler.getIntFromInput("Number of Values per Iteration: ")
            case 'normalize':
                return InputHandler.getBoolFromInput("Normalize Distributions? (Y/N): ")
            case _:
                return ''
    ...
    def generate(self):
        ...
        numValsPerIter = self['numberPerIteration']
        debug = self['debug']
        isNormalized = self['normalize']
        ...
        all_data = self.accessor['cppHandler'].getNumbers(numValsPerIter*numIterations)
        for n in range(numIterations):
            ...
            if (self['isScaleColorMap']):
                plt.imshow(frame, cmap=self['colorMap'], vmin=min_value, vmax=max_value)
            else:
                plt.imshow(frame, cmap=self['colorMap'], vmin=0.0, vmax=1.0)
            plt.title("Distribution Heat Map from " + self['algorithm'].upper())
            plt.xlabel("Random Int [0, " + str(numCandidates) + ") from " + self['algorithm'].upper())
            ...
            heatmapPath = 'heatmaps/distribution/' + str(self['algorithm']) + '_' + str(numCandidates) + 'x' + str(numIterations) + 'x' + str(numValsPerIter) + '_dist_HM_frame' + str(n) + '.png'
        ...
        for png_path in framePaths:
            ...
            frames.append(img)
        # generate .gif
        self['heatmapPath'] = 'heatmaps/'+ str(self['algorithm']) + '_' + str(numCandidates) + 'x' + str(numIterations) + 'x' + str(numValsPerIter) + '_dist_HM.gif'
