...
class Vis_ShadedRelief(VisualizationInterface):
    ...
    def getParamList(self):
        return ['rows/columns', 'colorMap', 'loopGif', 'gifDuration']
    ...
    def generate(self):
        ...
        # GENERATE data 
        array = self.accessor['cppHandler'].getNumbers(numRowsCols*numRowsCols)
        ...
        for frameNum in range(NUM_FRAMES):
            ...
            heatmapPath = 'heatmaps/shadedrelief/' + str(self['algorithm']) + '_' + str(numRowsCols) + '_shadedrelief_HM_frame' + str(frameNum) + '.png'
        ...
        self['heatmapPath1'] = 'heatmaps/' + str(self['algorithm']) + '_' + str(numRowsCols) + '_shadedrelief_heatmap.svg'
        plt.savefig(self['heatmapPath1'])
        ...
        for png_path in framePaths:
            ...
            frames.append(img)
        # generate .gif
        self['heatmapPath2'] = 'heatmaps/' + str(self['algorithm']) + '_' + str(numRowsCols) + '_shadedrelief_heatmap.gif'
        frames[0].save(self['heatmapPath2'], save_all=True, append_images=frames[1:], loop=(not self['loopGif']), duration=self['gifDuration']) # duration=gifDuration
    ...
    def open(self):
        cmd = 'open ' + self.params['heatmapPath1']
        run(cmd, shell=True)

        cmd = 'open ' + self.params['heatmapPath2']
        run(cmd, shell=True)
